...
namespace x86
{
	...
	int Context::uinstModificadas=0;
	int Context::uinstTotales=0;
	long long Context::numUinst=0;
...
void Context::EmitUinstEffectiveAddress(Uinst *uinst, int index, unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2)
{
	...
	new_uinst->valor=valor;
	new_uinst->numBytes=numBytes;
	new_uinst->flags=flags;
	new_uinst->numBytes2=numBytes2;
	new_uinst->valor2=valor2;


}


void Context::ParseUinstDep(Uinst *uinst, int index,unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2)
{
	// Regular dependence
	assert(misc::inRange(index, 0, Uinst::MaxDeps - 1));
	Uinst::Dep dep = uinst->getDep(index);
	...
	uinst->valor=valor;
	uinst->numBytes=numBytes;
	uinst->flags=flags;
	uinst->numBytes2=numBytes2;
	uinst->valor2=valor2;

}


void Context::ParseUinstIDep(Uinst *uinst, int index,unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2)
{
	// Get dependence
	...
	// Regular dependence
	ParseUinstDep(uinst, index,valor, numBytes, flags, numBytes2, valor2);
}

void Context::ParseUinstODep(Uinst *uinst, int index, unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2)
{
		...
		int numBytes2)
{
	...
	uinst->idUinst=Context::numUinst;
	Context::numUinst++;


	// Initialize
	uinst->setMemoryAccess(address, size);
	uinst->setIDep(0, idep0);//Fuente1
	uinst->setIDep(1, idep1);//Fuente2
	uinst->setIDep(2, idep2);//Flag
	uinst->setODep(0, odep0);//Destino
	uinst->setODep(1, odep1);//Flag
	uinst->setODep(2, odep2);//Flag
	uinst->setODep(3, odep3);//Flag
	...
	for (int i = 0; !uinst_effaddr_emitted && i < Uinst::MaxDeps; i++)
		EmitUinstEffectiveAddress(uinst.get(), i, valor, numBytes, flags, numBytes2, valor2);
	
	// Parse input dependences
	for (int i = 0; i < Uinst::MaxIDeps; i++)
		ParseUinstIDep(uinst.get(), i,valor, numBytes, flags, numBytes2, valor2);
	...
	for (int i = 0; i < Uinst::MaxODeps; i++)
		ParseUinstODep(uinst.get(), i,valor, numBytes, flags, numBytes2, valor2);
