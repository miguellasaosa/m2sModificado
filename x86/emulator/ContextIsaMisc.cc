...
//¿Cómo localizo lo que escribe en AUX, si es que se escribe realmente??
void Context::ExecuteInst_cmpxchg8b_m64()
{
	...
	newUinst(Uinst::OpcodeSub,
			...
			0,
			0,
			0,-1,0,flags);  // Compare edx-eax with m64
	newMemoryUinst(Uinst::OpcodeStore,
			getEffectiveAddress(),
			8,
			Uinst::DepZps,
	...
	newUinst(Uinst::OpcodeMove,
			...
			Uinst::DepEax,
			0,
			0,regs.getEdx(),4,-1,regs.getEax());  // Conditionaly store edx-eax
}
...
void Context::ExecuteInst_leave()
{
	...
	unsigned int valueAux=regs.getEbp();//he sido yo
	
	regs.setEsp(regs.getEbp());//OpCodeMove

	if (inst.getSegment())
		throw misc::Panic("Unsupported segment");

	//Se lee el valor de  @(esp), corresponde a 
	//OpcodeEffaddr,
	MemoryRead(regs.getEsp(), 4, &value);
	
	regs.incEsp(4);//OpCodeAdd
...
void Context::ExecuteInst_sahf()
{
	...
	newUinst(Uinst::OpcodeMove,
			...
			Uinst::DepZps,
			Uinst::DepCf,
			Uinst::DepOf,-1,0,regs.getEflags());
}
