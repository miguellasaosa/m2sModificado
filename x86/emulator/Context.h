...
public:
	...
	//Añadidos por mi para ver los porcentajes
	static int uinstModificadas;
	static int uinstTotales;
	static long long numUinst;
...
private:
	...
	// UinstMaxDeps - 1
	void EmitUinstEffectiveAddress(Uinst *uinst, int index,unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2);

	// Parse regular dependence, given as a global dependence index. The
	// value in \a index must be between 0 and UinstMaxDeps -1
	void ParseUinstDep(Uinst *uinst, int index,unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2);

	// Parse input dependences. Argument \a index is a value between 0 and
	// UinstMaxIDeps - 1
	void ParseUinstIDep(Uinst *uinst, int index,unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2);
	
	// Parse output dependence. Argument \a index is a value between 0 and
	// UinstMaxODeps - 1
	void ParseUinstODep(Uinst *uinst, int index,unsigned valor, int numBytes, int flags, unsigned valor2, int numBytes2);
