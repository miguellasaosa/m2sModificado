...
int Thread::op_effaddr=0;
int Thread::op_load=0;
int Thread::op_store=0;
int Thread::op_tot=0;
...
void Thread::tipoInstruccion(Uinst* inst){
	...
	if(std::strcmp(pch,"effaddr")==0) op_effaddr++;
	else if(std::strcmp(pch,"load")==0) op_load++;
	else if(std::strcmp(pch,"store")==0) op_store++;
	op_tot++;
}
