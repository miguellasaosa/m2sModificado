...
void Timing::DumpSummary(std::ostream &os) const
{
	...
	double porcentaje= (((double)x86::Context::uinstModificadas - (double)x86::RegisterFile::fallidas)/ (double)x86::Context::uinstTotales) * 100;
	os << "Operaciones cubiertas: "<<porcentaje<<" % \n\n";
	os << "Ilegales: "<<x86::RegisterFile::ilegales<<" \n\n";
	os << "fallidas: "<<x86::RegisterFile::fallidas<<" \n\n";
	os << "Totales: "<<x86::Context::uinstTotales<<" \n\n";
	os << "Cubiertas: "<<x86::Context::uinstModificadas<<" \n\n";

	double total=x86::RegisterFile::total_valores;

	std::string salida_res="/home/mlasaosa/tfg/resultados/"+benchmark+"/estadisticas";
	std::ofstream f1(salida_res);

	//11111111111111111111111111101100

	f1 << "ESTADISTICAS DE LOS VALORES \n";
	f1 << "--------------------------- \n";
	f1 << "Total: " <<total<<"\n\n";
	f1<<"POSITIVOS\n";
	f1<<"---------\n";
	f1 << "Valor 0: "<< x86::RegisterFile::v_0x0 << " ("<<(x86::RegisterFile::v_0x0/total)*100<<"%)\n";
	f1 << "Valor 1: "<< x86::RegisterFile::v_0x1 << " ("<<(x86::RegisterFile::v_0x1/total)*100<<"%)\n";
	f1 << "1 Byte: "<< x86::RegisterFile::de0x2a0xFF << " ("<<(x86::RegisterFile::de0x2a0xFF/total)*100<<"%)\n";
	f1 << "2 Byte: "<< x86::RegisterFile::de0x01FFa0xFFFF << " ("<<(x86::RegisterFile::de0x01FFa0xFFFF/total)*100<<"%)\n";
	f1 << "3 Byte: "<< x86::RegisterFile::de0x01FFFFa0xFFFFFF << " ("<<(x86::RegisterFile::de0x01FFFFa0xFFFFFF/total)*100<<"%)\n";
	f1 << "Hasta 0x7FFFFFFF: "<< x86::RegisterFile::de0x01FFFFFFa0x7FFFFFFF << " ("<<(x86::RegisterFile::de0x01FFFFFFa0x7FFFFFFF/total)*100<<"%)\n\n";

	f1<<"NEGATIVOS\n";
	f1<<"---------\n";
	f1 << "Valor -1: "<< x86::RegisterFile::v_0xFFFFFFFF << " ("<<(x86::RegisterFile::v_0xFFFFFFFF/total)*100<<"%)\n";
	f1 << "1 Byte: "<< x86::RegisterFile::de0xFFFFFFFEa0xFFFFFF00 << " ("<<(x86::RegisterFile::de0xFFFFFFFEa0xFFFFFF00/total)*100<<"%)\n";
	f1 << "2 Byte: "<< x86::RegisterFile::de0xFFFFFE00a0xFFFF0000 << " ("<<(x86::RegisterFile::de0xFFFFFE00a0xFFFF0000/total)*100<<"%)\n";
	f1 << "3 Byte: "<< x86::RegisterFile::de0xFFFE0000a0xFFFFFF00 << " ("<<(x86::RegisterFile::de0xFFFE0000a0xFFFFFF00/total)*100<<"%)\n";
	f1 << "Hasta 0x7FFFFFFF: "<< x86::RegisterFile::de0xFE000000a0x80000000 << " ("<<(x86::RegisterFile::de0xFE000000a0x80000000/total)*100<<"%)\n\n";



	f1<<"TIPOS DE INSTRUCCIONES\n";
	f1<<"---------\n";
	f1 << "Instrucciones Load: "<< ((double)x86::Thread::op_load / (double)x86::Thread::op_tot)*100 <<"% \n";
	f1 << "Instrucciones Store: "<< ((double)x86::Thread::op_store / (double)x86::Thread::op_tot)*100 <<"% \n";
	f1 << "Instrucciones EffAddr: "<< ((double)x86::Thread::op_effaddr / (double)x86::Thread::op_tot)*100 <<"% \n";
	f1 << "Resto de instrucciones: "<< (((double)(x86::Thread::op_tot - (x86::Thread::op_load + x86::Thread::op_store + x86::Thread::op_effaddr)))/(double)x86::Thread::op_tot)*100 <<"%\n\n";
	...
	long total_flips[32];
	long long total_timeBit0[32];
	for (int i=0; i<32; i++){
		total_flips[i]=0;
		total_timeBit0[i]=0;	
	}

	long long lastCycleAux;
	for(int i=0; i<x86::RegisterFile::integer_local_size ; i++){
		for(int j=0; j<32 ;j++){
			if(x86::RegisterFile::resultados.getValue(i,j)==0){
				lastCycleAux=x86::RegisterFile::resultados.getLastCycle(i,j);
				x86::RegisterFile::resultados.setTimeBit0(i,j,cpu->getCycle()-lastCycleAux);
			}
		}
	}

	for(int i=0; i<32; i++){
		for(int j=0; j<x86::RegisterFile::integer_local_size ; j++){
			total_flips[i]+=x86::RegisterFile::resultados.getFlip(j,i);
			total_timeBit0[i]+=x86::RegisterFile::resultados.getTimeBit0(j,i);
		}
	}

	//Escribe los resutados en los registros.
	for(int i=0; i<x86::RegisterFile::integer_local_size ; i++){
		//std::string fichero="/home/mlasaosa/tfg/resultados/"+benchmark+"/reg"+std::to_string(i);
		std::string fichero="/home/mlasaosa/tfg/resultados/"+benchmark+"/reg"+std::to_string(i);
		std::ofstream f(fichero);
		f<<"REGISTRO FISICO "<<i<<"\n";
		f<<"celda\tflips\tciclos a 0\n";
		for(int j=0; j<32 ;j++){
			f<<j<<"\t"<<x86::RegisterFile::resultados.getFlip(i,j)<<"\t"<<x86::RegisterFile::resultados.getTimeBit0(i,j)<<"\n";
		}
		f.close();
	}

	std::string fichero="/home/mlasaosa/tfg/resultados/"+benchmark+"/resumen";
	std::ofstream f(fichero);
	f<<"Celda\tFlips\tTiempo bit 0\tDuty cycle 0\n";
	for(int i=0; i<32; i++){
		f<<i<<"\t"<<total_flips[i]<<"\t"<<total_timeBit0[i]<<"\t\t"<<((double)total_timeBit0[i]-6*cpu->getCycle())/((double)cpu->getCycle()*180)<<"\n";
	}
	f<<"\nCiclos Totales: "<<cpu->getCycle()<<"\n";
}
...
void Timing::setBenchmark(std::string nombre, int tam){
	for(int i=0; i<tam; i++){
		benchmark+=nombre[i];
	}
}
